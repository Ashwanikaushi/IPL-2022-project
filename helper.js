...
function fileCreater(playerPath, dataObj){
    let arr = [dataObj];
    fs.writeFileSync(playerPath, JSON.stringify(arr));// js me khali arr yha chalega hi nhi to strigify ya arr ke peeche or kuch krna hi padega.
}
function fileUpdater(playerPath, dataObj){
    // file content read 
    let dataBuffer = fs.readFileSync(playerPath);
    // buffer -> se convert hui json me , taki json entry add ho
...
function excelReader(filePath) {
    ...
    // sheet -> json ( sheet se convert kiya json me)
    let ans = xlsx.utils.sheet_to_json(excelData);
    return ans;
}
function excelWriter(filePath, json  ) {
    // console.log(xlsx.readFile(filePath));
    // empty workbook banayi
    let newWB = xlsx.utils.book_new();
    //  worksheet -> phr isme data put kiya player ka
    let newWS = xlsx.utils.json_to_sheet(json);
    // wb -> put worksheet -> worksheet name -> sheet1(player name)
